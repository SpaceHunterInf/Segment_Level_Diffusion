...
import CONSTANTS
from diffusion.sentence_denoising_diffusion import GaussianDiffusion, Trainer
...
def main(args):
    ...
    #sequence length for diffusion transformer
    
    #assert args.max_diffusion_len == args.max_sentence_len * args.num_of_sentences
    model = DiffusionTransformer(
        tx_dim = args.tx_dim,
        tx_depth = args.tx_depth,
        heads = args.tx_dim//ATTN_HEAD_DIM,
        latent_dim = latent_dim,
        max_seq_len = args.max_diffusion_len,
    ...
    if args.eval_test:
        trainer.load(args.resume_dir, best=trainer.diffusion.diffusion_model.seq2seq)
        #breakpoint()
        if args.decoding_reload_folder != None:
            trainer.load_enc_dec(args)
        #breakpoint()
        if trainer.diffusion.diffusion_model.seq2seq:
    ...
    if args.resume_training:
        print(f'resuming training to steps{args.num_train_steps}')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_output_seq_length", type=int, default=64)
    parser.add_argument("--max_input_seq_length", type=int, default=512)
    parser.add_argument("--num_of_sentences", type=int, default=2)
    parser.add_argument("--max_diffusion_len", type=int, default=128)
    parser.add_argument("--parallel_encoding", action='store_true', default=False)
    parser.add_argument("--parallel_decoding", action='store_true', default=False)
    parser.add_argument("--direct_connection", action='store_true', default=False, help='No compression/reconstruction if true')
    parser.add_argument("--specified_context_encoder", type=str, default=None, help='separate context encoder path')
    parser.add_argument("--decoding_loss", action='store_true', default=False, help='adding decoding loss')
    parser.add_argument("--decoding_loss_weight", type=float, default=1.0, help='decoding loss weight')
    parser.add_argument("--decoding_reload_folder", type=str, default=None)
    parser.add_argument("--post_ae_loss", action='store_true', default=False, help="add reconstruction loss at higher dimension")
    parser.add_argument("--post_ae_loss_weight", type=float, default=1.0, help='post ae loss weight' )
    parser.add_argument("--specified_tokenizer", type=str, default=None, help='separate tokenizer path')
    
    args = parser.parse_args()
    # Remove hardcoded API key - use environment variable instead
    
    wandb.init(
        project=os.getenv("WANDB_PROJECT", "xsum_diffusion"),
        name=args.wandb_name if args.wandb_name and len(args.wandb_name) > 0 else args.dataset_name,
    )
    ...
    if args.eval or args.resume_training or args.eval_test:
        ...
        heldout_params = {'decoding_loss','num_train_steps', 'save_and_sample_every', 'wandb_name', 'output_dir', 'resume_dir', 'eval', 'eval_test', 'num_samples', 'sampling_timesteps', 'sampling_schedule', 'seq2seq_candidates', 'scale', 'sampler', 'resume_training'}
    ...
    #assert args.decoding_loss == True
    main(args)
